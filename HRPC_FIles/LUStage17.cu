...
__global__ void GpuLUCumputeStage1(float* matrix, uint32_t n, uint32_t k)
{
    uint32_t startOfBlock = (gridDim.x * blockIdx.y + blockIdx.x) * (blockDim.x * blockDim.y);
    uint32_t threadIndex = startOfBlock + (blockDim.x * threadIdx.y + threadIdx.x);
    uint32_t totalGridWidth = gridDim.x * blockDim.x;
    uint32_t i = threadIndex / totalGridWidth;
    uint32_t j = threadIndex % totalGridWidth;
    i = i + (k + 1);
    j = j + (k + 1);
    threadIndex = i * n + j;
    //if (i * n + j != threadIndex)
    //{
    //    // printf("^");
    //}
    ...
    if (i < n && j < n)
    {
        // printf("#");
        float toBeL = matrix[i * n + k] / matrix[k * n + k];
        matrix[threadIndex] -= (toBeL * matrix[k * n + j]);
    }
}

__global__ void GpuLUCumputeStage2(float* matrix, uint32_t n)
{
    uint32_t startOfBlock = (gridDim.x * blockIdx.y + blockIdx.x) * (blockDim.x * blockDim.y);
    uint32_t threadIndex = startOfBlock + (blockDim.x * threadIdx.y + threadIdx.x);
    uint32_t i = threadIndex / n;
    uint32_t j = threadIndex % n;

    if (i > j && i < n)
    {
        float toBeL = matrix[threadIndex] / matrix[j * n + j];
        matrix[threadIndex] = toBeL;
    }
}
...
void computeLUMatrixGpuCompact(float* matrix, const uint32_t n, uint32_t maxNThreadsPerBlock)
{
    ...
    {
        uint32_t nJsNeeded = n - (k + 1);
        uint32_t nBlock = nJsNeeded / nT;
        if (nJsNeeded % nT != 0)
        {
            nBlock++;
        }

        dim3 grid(nBlock, nBlock);
        // printf("k: %d, n: %d, n - (k + 1): %d, nT: %d, nBlock: %d.\n", k, n, n - (k + 1), nT, nBlock);
        GpuLUCumputeStage1 << <grid, blockT>> > (matrix, n, k);
        cudaDeviceSynchronize();
        gpuErrchk(cudaPeekAtLastError());
    }

    uint32_t nJsNeeded = n - 1;
    uint32_t nBlock = nJsNeeded / nT;
    if (nJsNeeded % nT != 0)
    {
        nBlock++;
    ...
    dim3 grid(nBlock, nBlock);
    // printf("n: %d, n - 1: %d, nT: %d, nBlock: %d.\n",n, n - 1, nT, nBlock);
    GpuLUCumputeStage2 << <grid, blockT >> > (matrix, n);
    cudaDeviceSynchronize();
    gpuErrchk(cudaPeekAtLastError());
