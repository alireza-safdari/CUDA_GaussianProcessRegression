...
const uint32_t SHARED_MEMORY_SIZE = N_THREADS_PER_BLOCK * 1; // SHARED_MEMORY_SIZE = N_THREADS_PER_BLOCK * N, N >= 1 and integer
...
__global__ void GpuLUCumputeRow(float* matrix, const uint32_t n, uint32_t k, uint32_t iStart, uint32_t jStart, uint32_t nColumn, uint32_t nColumnForMemoryRead)
{
    ...
    __shared__ float kRow[SHARED_MEMORY_SIZE];


    // using grid stride to fill it up
    for (uint32_t matrixIndex = k * n + jStart, columnIndex = 0; columnIndex + blockDim.x - 1 < nColumnForMemoryRead; matrixIndex += blockDim.x, columnIndex += blockDim.x)
    {

        uint32_t kRowIndex = columnIndex + threadIdx.x;
        kRow[kRowIndex] = matrix[matrixIndex + threadIdx.x];

        //if ((threadIdx.x == 0 || threadIdx.x == blockDim.x - 1))
        //{
        //    printf("Id: %d, kRowIndex: %d, , matrixIndex: %d, columnIndex: %d, jStart: %d, blockDim: %d, nColumnForMemoryRead: %d\n", threadIdx.x, kRowIndex, matrixIndex + threadIdx.x, columnIndex, jStart, blockDim.x, nColumnForMemoryRead);
        //}
    }
    ...
    {
        matrix[i * n + j] -= (toBeL * kRow[columnIndex]);
    }
}
...
void calculateNColumnValues(uint32_t& nColumn, uint32_t& nColumnForMemoryRead, const uint32_t n, const uint32_t j, const uint32_t nThreadsNeeded)
{
    nColumn = n - j;
    ...
    nColumnForMemoryRead = nColumn;
    if (nColumnForMemoryRead < SHARED_MEMORY_SIZE || nThreadsNeeded < nColumnForMemoryRead)
    {
        if (nColumnForMemoryRead % nThreadsNeeded != 0)
            nColumnForMemoryRead = ((nColumnForMemoryRead / nThreadsNeeded) + 1) * nThreadsNeeded;
        if (nColumnForMemoryRead > SHARED_MEMORY_SIZE)
        {
            nColumnForMemoryRead -= nThreadsNeeded;
            nColumn = nColumnForMemoryRead;
        }
    }
}
        ...
        for (uint32_t i = k + 1; i < n;)
        {
            if (nThreadsNeeded >= maxNThreadsPerBlock) // 1024 because it is maximum thread allowed per Multiprocessor
            {
                for (uint32_t j = k + 1; j < n; j += SHARED_MEMORY_SIZE)
                {
                    ...
                    calculateNColumnValues(nColumn, nColumnForMemoryRead, n, j, maxNThreadsPerBlock);
                    // printf("if: nThreadsNeeded: %d, k: %d, i: %d, j:%d, nColumn: %d, nColumnForMemoryRead: %d\n", nThreadsNeeded, k, i, j, nColumn, nColumnForMemoryRead);
                    GpuLUCumputeRow << <1, maxNThreadsPerBlock, 0, gpuStreams[streamIndex] >> > (matrix, n, k, i, j, nColumn, nColumnForMemoryRead);
            ...
            else
            {
                ...
                {
                    uint32_t nColumn, nColumnForMemoryRead;
                    calculateNColumnValues(nColumn, nColumnForMemoryRead, n, j, nThreadsNeeded);
                    // printf("else: nThreadsNeeded: %d, k: %d, i: %d, j:%d, nColumn: %d, nColumnForMemoryRead: %d\n", nThreadsNeeded, k, i, j, nColumn,nColumnForMemoryRead);
                    GpuLUCumputeRow << <1, nThreadsNeeded, 0, gpuStreams[streamIndex] >> > (matrix, n, k, i, j, nColumn, nColumnForMemoryRead);
                    streamIndex = incrementStreamIndex(streamIndex);
                    gpuErrchk(cudaPeekAtLastError());
